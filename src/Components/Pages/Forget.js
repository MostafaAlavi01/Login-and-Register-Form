...
const Forget = () => {
  ...
  useEffect(() => {
    setErrors(validate(data,'Recovery'));
  }, [data]);
  ...
  const submitHandler =(event)=>{
    ...
    if(!Object.keys(errors).length){
        notify("Reset Link send to your email","success")
    }else{
      ...
      <div className="  md:w-1/2 max-w-sm  bg-main-gray mt-14">
        ...
        <form onSubmit={submitHandler} className=" p-10 rounded-lg  min-w-full">
          <h1 className="text-center text-1xl mb-6 text-white">
            Please enter your email to recovery password.
          </h1>
          <div>
        ...
        <div className="flex justify-center py-5 ">
          ...
          <Link to="/login" className="w-38 h-10 px-4 ml-1  text-white ">
            Login
          </Link>
        </div>
      </div>
